...
set wildignore+=.git,.svn,.DS_Store,.npm,.vagrant,*.zip,*.tgz,*.pdf,*.psd,*.ai,*.mp3,*.mp4,*.bmp,*.ico,*.jpg,*.png,*.gif,*.epub,.hg,.dropbox,.config,.cache,*.pyc,node_modules/*,bower_components/*,*.min.*,_*,target,bazel-*
...
" For some reason proto.vim doesn't set this
au FileType proto setlocal sts=2 sw=2 ts=2 et tw=80
au FileType bzl setlocal sts=4 sw=4 ts=4 et

command! MakeTags !ctags -R .
"command! MakeTags !uctags -R .

"default yank buffer = window manager clipboard
...
let g:rooter_patterns = ['Cargo.lock', 'WORKSPACE', '.git/', 'WORKSPACE.bazel']
...
"noremap gw :call FzfTagsCurrentWord()<cr>
nnoremap <silent> <Leader>rg :Rg <C-R><C-W><CR>

autocmd FileType c,cpp,java,php,rust,toml,sh autocmd BufWritePre <buffer> %s/\s\+$//e
